  ...
  try {
    ...
    if (res.status === 200) {
      ...
      await Promise.all(promises).catch((err) => {
        throw new Error(`fetchTemplatesFromGithub: ${err.message}`);
      });
...
exports.downloadTemplateFromGithub = async function downloadTemplateFromGithub(targetPath, config, url) {
  try {
    ...
    if (res.status === 200) {
        ...
        return new Promise((resolve, reject) => {
          if (item.download_url && item.type === "file") {
            axios
              ...
              .then((res) => {
                res.data.pipe(fs.createWriteStream(`${targetPath}/${realPath}`));
                resolve();
              })
              .catch(reject);
      ...
      await Promise.all(promises).catch((err) => {
        throw new Error(`downloadTemplateFromGithub error: ${err.message}`);
      });
    } else {
